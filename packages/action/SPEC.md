...
## 模块宪法要点
- `runtime.ts`:同一工具可并发,文件写操作串行(互斥队列);write/edit 走临时文件 + rename 原子提交;取消/超时终止整棵进程树,后台任务取消时清理孤儿
- `capability.ts`:询问时发的 `permission_request` 事件必须带工具名/能力/理由 + **参数摘要**(bash 命令全文、write 目标路径)——用户批准前看到"模型要跑什么",不只是"要不要放行"
