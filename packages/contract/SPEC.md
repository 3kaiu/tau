...
- 工具分级:tools[] 带 `tier`(T0 常驻 / T1 按需经 `tool:catalog` 查询后注入本 turn)——每轮工具描述 token 只花在会用到的
...
- `Command` 封闭联合(prompt/steer/approve/**deny**/answer/abort/select/observe),**统一携带 `sender{clientId, kind}`**;answer 携带 `questionId`;**deny 与 ApprovalState 全态对齐**(active/approved/denied/expired/revoked 均可达,用户"明确拒绝"有命令表达,deny 也带 sender 审计);observe = 只读 attach(订阅观察,多窗口/远程)
- `Event` 封闭联合(transcript/tool/权限/压缩/生命周期/**input_accepted**/**budget_exceeded**/**loop_detected**/**retry**/**model_switched**/**interrupted**/**recovery**),事件带 `id`(因果/幂等/重放);**id 生成规则归属:权威写入侧(session)生成,格式 = 进程内单调序列 + 进程前缀;跨源/跨进程排序按 (epoch, id) 字典序**,重放与 multi-run 下因果序可判定
...
8. **命令可审计**:一切 Command 携带 sender;approve/answer/abort 强制,审计到人
