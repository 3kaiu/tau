...
6. **两级装载**:目录级信息(技能名+一句话、user 级资源摘要、记忆索引)常驻 system;大资源全文(AGENTS.md/skill 正文/记忆全文)按需取(`skill:load`/`memory_read`/`memory_search`),绝不常驻。记忆索引是**构造期快照**(写入后不实时刷新,会话创建/恢复时经 `apply(sessionId)` 重建)
