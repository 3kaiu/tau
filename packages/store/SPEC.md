...
7. **保留策略**:会话归档(完成会话 `session.archive()` 置 archived 状态)、审计滚动窗口(`archiveAudit` 移至 `audit_archive` 表,不删历史)、artifact 配额(M6 随 artifacts 模块定案)--事件表/审计表/artifact 正文三条增长线都有去处,不无限膨胀
...
- `migrate.ts`:迁移幂等(forward-only,不回滚);版本号记在 kv 表;首次迁移建全量表(sessions/messages/events/audit/audit_archive + 索引)
...
- 索引齐全:消息按 (session, seq) 索引、事件按 (session, seq) 索引、审计按 (session, timestamp) 索引
