...
- **P1-21 orchestrate 恢复链** ✅(M10.1 收尾):store v5 审计带 turnId;action 全审计路径透传;orchestrate lifecycle.ts 落地(LoopGuard 行为指纹 + turnIdOf 提交锚)+ turn 尾部 commitTurn;session 恢复侧悬置判定(uncommittedSyscalls 纯函数,未提交 syscall 清单进 recovery 事件 detail 与投影告警,旧数据退回通用告警,已提交 turn 不误报);eval #10 重写 + 新增 #23。
- **P1-22 action API 形态** ✅ 口径修正:`permissionRequest()`/`grant(requestId)` 已在(权限双轨时落地);新增 `grantScope(caps, scope)` 作用域预授权(一次批准 N 次,maxUses/durationMs,危险命令不豁免,落审计);`Stream<ToolEvent>` 形态 M10.2 落地:`executeStream` 为底层原语(逐调用产出 `started` → `completed`/`failed` 终态事件,结果/错误在终态),`execute` 改兼容收口(内部 for-await 流消费,行为不变);权限询问/挂起等旁路事件不进流,仍经 onEvent 双轨(全局桥与流不互斥,同一事件两侧可见);eval 新增 #24(成功/失败终态与 execute 收口一致 + 双轨同序),action 单测 4 例(成功/失败/门拒绝/双轨)。
...
P1-21/P1-22 相关"(规划)"标注全部清除(action 功能面/模块宪法);M10.3-a 起逐项消化剩余 P2 开放项(steer 立即断流 ✅:action `ExecuteRequest.signal` 中断输入 + orchestrate `steer({interrupt:"immediate"})`,eval #25 + 单测 3 例;artifacts 大载荷外置 ✅:store v6 artifacts 表 + session artifacts.ts 自动外置 + action artifact:read 工具,eval #26;loader 增量装载 ✅:enhance LoaderCache(mtime/size 命中不重读)+ loaderStats 可观测 + AGENTS.md 同走缓存,eval #27;workspace 文件树增量索引 ✅:action WorkspaceIndex(目录 mtime 失效不全量重扫,find 接入,freshness 不牺牲),eval #28;Config tier 注入裁剪 ✅:contract config.ts 去"(规划)"(schema 已实现),session `requestTools` 按 tier 裁剪投影工具(T0 常驻 + tool:catalog + 本 turn T1,缺省全量)、orchestrate 用过即请求、beginTurn 重置,eval #29;compaction 文件拆分 ✅:session src/compaction.ts `runCompact` 独立编排,compact 委托,行为零漂移)。**M10.3 剩余 P2 开放项全部消化完毕(a–f 六项),audit8 开放项清零。** 后续:contract Config 装载/合并/消费方 ✅(M10.4):audit8 唯一的遗留标注(contract P?/app SPEC Config 消费方"随 M9 定案")收口——contract `coerceConfigValue`/`parseMergedConfig`/`formatConfigError` 装载配套;app `src/config.ts` 装载路径 + `tau config set` 已知键强转校验(非法退出 2)+ compose `configStore`/`config` 消费(maxContextTokens/toolTierRules 透传 session),eval #30;**全仓库 "(规划)" 无残余。** M10.5 action workspace 待办消化(PLAN M10.3-d 遗留三项):gitignore 忽略树并入(root .gitignore 预编译匹配树 + {mtimeMs,size} 指纹失效整根重扫,目录型 pattern 对目录本身补查 rel+"/");越界校验迁移(PathBoundary 删除 → `WorkspaceIndex.resolveWithin` 统一,roots 空不设界/非空严格拒绝,read/write/edit/grep/find/ls 全线接入);Multi-run worktree 归属(worktree.ts T2 内部件 create/rm/list + `.tau-worktrees` 入 SKIP_DIRS + multirun 子会话迁独立工作树,finally 必清,`RunResult.cwd` 可断言);tier 三态 T0/T1/T2(contract 三 schema + projector 两分支均排 T2 永不注入);eval 扩 #16 + 新增 #31;**全仓库 "(规划)" 无残余(PLAN 剩余开放项 = 无)。** M11 认知与长程记忆 ✅:enhance memory.ts 真实化(kv 前缀持久 + overwrite 保护 + listMemory/searchMemories 时间衰减打分)、contract SystemKind + "memory"、apply(sessionId) 记忆索引块(priority 30、≤20×60 预览截断、空会话无块、构造期快照)、app memory:* 五工具面,enhance/app/contract SPEC 同步,eval #32。M12 多代理编排深化 ✅:orchestrate subagent.ts(fork/join/background/limiter/深度/注册表/独立 worktree/capability 递减拒绝落审计)、**action bypassQueue 逃逸口**(T0 写队列死锁修复:subagent:run 占队时子代理工具不排父队列)、**compose registerSyscall 统一注册**(defaultRule 声明必进能力门,skill_load/memory/subagent 收敛)、app subagent:run syscall(T0 危险缺省 ask),orchestrate/app SPEC 更新(subagent.ts 真实落点),eval #33。M10 插件市场生态决策 ✅:撤销候选 → 明确不做(自举工具无社区,市场是负资产;扩展面 = skills 目录 + AGENTS.md + MCP 已闭环);保留 opencode plugin API 兼容 + TrustLevel 作 import 红利;PLAN 候选行/决策表 D4/风险表 + enhance SPEC 宪法 3/模块宪法同步。

技术债逐项消化(用户点名逐项还,2026-08):**高危·提交断线** ✅——63 文件分 3 批提交(M10.1-10.5 四包 → M11+M12 → app+决策),工作区干净;此前 8 次会话零提交的断线风险清除。**中危 7 项全部消化** ✅:#1 store 单写者锁(`src/lock.ts` StoreLock:`.lock` wx 独占 + pid 存活判定接管崩溃残留;`:memory:`/readonly 不拿锁,close 释放;SqliteStoreOptions.readonly + app cli openStore 只读;单测 +3);#2 压缩触发缺省(contract triggerRatio=0.8 → scheduler maybeCompact 0.7→0.8 消费 + session compactionKeepRecent 缺省 6 + app 透传);#3 thinking 上限(session appendMessage 截断,缺省 32KB 对齐 ThinkingPolicySchema.maxBytes,保留头部 + 超限标记;maxThinkingBytes 可配;单测 +1);#4 事件流泄漏(admit input_accepted 回执与 externalizeContent 同阈值,大输入只进事件流 200 字符预览 + 外置标记,正文只存 store);#5 session SPEC "queue 不走 Session 接口"(排队在 orchestrate 内部 steerQueue);#6 session SPEC "摘要回调注入点在 scheduler CompactStrategy.summarize,无纯规则回退";#7 contract SPEC "投影按 tier 裁剪,能力裁剪为可选项(现状)"。**低危文档漂移批量对齐** ✅:contract(幂等语义位/事件 id 现状 randomUUID/不可变措辞/JSON Schema 运行期生成/T2 档/image 块)、session(归档 O(n) 现状/双端惰性装载为性能目标未落地/retrieveFrom 导出面)、store(createStore 缺省 :memory: 回退/单写者锁兑现/artifact 配额与 FTS5 规划标注)、eval(asserts.ts/fixtures.ts 单文件化,空目录删除)、surface/tui 模块表补 print.ts/theme.ts/index.ts、action 模块表 14+3 工具数对账、contract docs/semantics.md 补 ApprovalState/thinking+artifact/DangerousCommandPatterns 三节。验证:`bun run check` 零告警、`bun test` 330 tests / 0 fail、`bun run eval` 33/33 passed。
