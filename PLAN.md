...
- M9 方向提案(docs/M9.md)+ 骨架已立:`tau doctor` 扩展为 4 项自检(模型目录/凭据/契约 wire 往返/store 可达+replay/capability 门生效)、新增 `tau log <sessionId>`(事件流 JSONL)与 `tau replay <sessionId>`(重放→投影→人类可读转述,复用 surface 渲染器);出口断言草案 #19-#22 待 M9 全量落地时补;export/会话治理/单二进制分发待做;候选 M10 插件市场生态 / M11 认知与长程记忆 / M12 多代理编排深化
