...
- 技术债逐项消化 ✅(M12 后用户点名逐项还债,1 高危 + 7 中危 + 低危文档漂移):**高危·提交断线**——63 文件分 3 批安全提交(M10.1-10.5 core+contract/store/session/action → M11+M12 → app+决策);**中危 #1 store 单写者锁**——`src/lock.ts` `StoreLock`(`.lock` 文件 `wx` 独占 + pid 存活判定接管崩溃残留;`:memory:`/readonly 不拿锁,close 释放;SqliteStoreOptions.readonly 落地,app cli openStore 改只读;单测 +3);**中危 #2 压缩触发缺省**——contract `triggerRatio=0.8` 找到消费方(scheduler `maybeCompact` 0.7→0.8)+ session `compactionKeepRecent`(缺省 6)+ app 透传;**中危 #3 thinking 上限**——session appendMessage thinking 块截断(缺省 32KB 对齐 ThinkingPolicySchema.maxBytes,保留头部 + 超限标记,`maxThinkingBytes` 可配;单测 +1);**中危 #4 事件流泄漏**——admit 的 input_accepted 回执与 externalizeContent 同阈值(大输入只进事件流 200 字符预览 + 外置标记,正文只存 store);**中危 #5 session.queue 声称**——SPEC 改"steer/follow-up 排队在 orchestrate 内部 steerQueue";**中危 #6 纯规则截断声称**——SPEC 改"摘要回调注入点在 scheduler CompactStrategy.summarize,session 不调 LLM 无回退分支";**中危 #7 capabilities 裁剪声称**——contract SPEC 改"投影按 tier 裁剪、能力裁剪为可选项(现状)";低危文档漂移批量对齐(contract:幂等语义位/事件 id 随机 UUID 现状/不可变-编译期 JSON Schema 措辞/T2 档/image 块;session:归档 O(n) 现状/双端惰性装载为性能目标未落地/retrieveFrom 导出/检索来源三字改两字;store:createStore 缺省回退 :memory:/单写者锁兑现细节/artifact 配额与 FTS5 规划标注;eval:模块表 asserts.ts/fixtures.ts 单文件化 + 空目录删除;surface/tui 模块表补 print.ts/theme.ts/index.ts;action 模块表 14+3 工具数对齐;contract docs/semantics.md 补 ApprovalState/thinking+artifact/DangerousCommandPatterns 三节);全量 330 测用例 0 fail,`bun run check` 零告警,`bun run eval` 33/33 passed(技术债条目见 docs/audit8.md 收尾节)
